...
<template>
  <div class="tool">
    ...
    <a-tooltip placement="topLeft" arrow-point-at-center>
      ...
      <a-icon type="bulb" class="tool-tip icon" @click="changecolor" />
    </a-tooltip>
    <!-- 消息列表 -->
    ...
    <!-- 换主题 -->
    <a-icon type="skin" class="tool-skin icon" title="更换主题" @click="showBackgroundModal = true" />
    <!-- github -->
    <a v-if="isDemo" href="https://github.com/littlewh/Hyperion_Chatroom" title="前往Github" target="_blank" class="tool-github icon">
      <a-icon type="github" />
    </a>
    <!-- 退出 -->
    <a-icon class="tool-out icon" type="poweroff" title="退出" @click="logout" />
    ...
    <a-modal title="主题" :visible="showBackgroundModal" footer="" @cancel="showBackgroundModal = false">
      <div class="tool-user-info">
        <div class="tool-user-title" style="width: 65px">
          ...
          <a-tooltip placement="topLeft" arrow-point-at-center>
            <div slot="title">
              <span>请输入以.jpg或.jpeg或.png或.gif结尾的链接</span>
            </div>
...
import { DEFAULT_BACKGROUND } from '@/images/index';
...
export default class Tool extends Vue {
  ...
  created() {
    this.username = this.user.username;
    const color = window.localStorage.getItem('back-color');
    if (color) {
      document.documentElement.setAttribute('back-color', color);
    }
  }
  ...
  changecolor() {
    const color = window.localStorage.getItem('back-color');
    if (color === 'day') {
      window.localStorage.setItem('back-color', 'night');
      document.documentElement.setAttribute('back-color', 'night');
    } else {
      window.localStorage.setItem('back-color', 'day');
      document.documentElement.setAttribute('back-color', 'day');
    }
  }
