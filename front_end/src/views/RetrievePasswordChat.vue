<template>
  ...
  >
    ...
    <!-- 登录注册 -->
    <Login @retrieve="handleRetrieve" @login="handleLogin" :showModal="showModal"></Login>
...
import Login from '@/components/Retrieve.vue';
...
export default class Chat extends Vue {
  ...
  // 找回密码
  async handleRetrieve(email: Email) {
    ...
    if (email.code == undefined || email.code === '') {
      ...
      const res = await this.retrieve(email);
      if (res) {
        // 进入系统事件
        // await this.handleJoin();
        this.clearUser();
        this.$router.go(1); // 登录页面
      }
    }
  }
